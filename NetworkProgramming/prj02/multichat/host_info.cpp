...
char *host_info_get_p_host_ip() {
	return p_host_info->host_ip;
}

static bool host_info_is_host_ip_inited() {
	return p_host_info->host_ip[0] != 0;
}

void host_info_set_host_ip(char *name, char *ip) {
	if(strcmp(p_host_info->name, name) != 0) return;
	if(!host_info_is_host_ip_inited()) return;

	strcpy(p_host_info->host_ip, ip);
}
...
bool host_info_is_overlapped_name(char *name, char *ip) {
	return strcmp(p_host_info->name, name) == 0
		&& host_info_is_host_ip_inited()
		&& strcmp(p_host_info->host_ip, ip) != 0;
}
